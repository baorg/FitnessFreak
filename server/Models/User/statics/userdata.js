...
async function getUserData(user_id) {
    let user = await this.findOne({ _id: mongoose.Types.ObjectId(user_id) })
        .select(['username', 'first_name', 'last_name', 'created_at', 'profile_image', 'banner_image', 'bio', 'score', 'chosen_category'])
        .exec();
    return user;
}
...
async function findUserByEmail(email) {
    let user = await this.findOne({ email })
        .select(['username', 'email', 'first_name', 'last_name', 'created_at', 'profile_image', 'bio', 'score', 'chosen_category'])
        .exec();
